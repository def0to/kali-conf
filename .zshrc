...
configure_prompt() {
    ...
    LOCAL_IP="%F{green} %F{yellow}$ip%F{green}"
    # Skull emoji for root terminal
    #[ "$EUID" -eq 0 ] && prompt_symbol=💀    case "$PROMPT_ALTERNATIVE" in
        case "$PROMPT_ALTERNATIVE" in

        twoline)
            PROMPT=$'%F{%(#.blue.green)}┌─${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}%n'$prompt_symbol$'%m%b%F{%(#.blue.green)})-[$LOCAL_IP]-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]\n└──╼%B%(#.%F{red}#.%F{blue}$ %b%F{reset}'
